...
`--` ค่าตำแหน่งอากาศ เหรียญ หุ้น และ**ที่อยู่ bridge** แก้บนเครื่องทีหลังผ่าน
...
แก้ตำแหน่งอากาศ เหรียญ หุ้น และที่อยู่ bridge จากเบราว์เซอร์ — ไม่ต้องแฟลชใหม่
...
ตั้งที่อยู่ **Bridge** แล้วกด **Save & reboot** หากต้องออกโดยไม่มีเบราว์เซอร์
กดค้างที่ไหนก็ได้บนจอเพื่อออกโดยไม่บันทึก

ช่อง **Bridge** ใส่ที่อยู่เครื่องที่รัน Claude Code — พิมพ์ IP เปล่า
(`192.168.1.20`) ก็พอ ระบบเติม `http://` กับพอร์ต `:8787` ให้เอง พอออกจากช่อง
(blur) อุปกรณ์จะยิง `/quota` ไปเช็คว่าปลายทางเป็น bridge จริง — เขียว ✓ คือถึงแล้ว
แดง ✗ (`no response` `HTTP 404` `not the bridge`) คือยังไม่ถึง **บันทึกเฉพาะที่อยู่
สีเขียวเท่านั้น** ที่อยู่ผิดหรือยิงไม่ติดจะคงค่า bridge เดิมไว้ ดังนั้นเครื่อง Claude
ต้องเปิดอยู่ตอนตั้งค่านี้ ย้าย Mac ทีหลังก็แก้ในพอร์ทัลนี้ ไม่ต้องแฟลชใหม่
(จอง DHCP reservation ให้ Mac จะได้ไม่ต้องแก้บ่อย)
