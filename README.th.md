...
> **การอ่านตัวเลขไม่มีต้นทุน** ตัวเลขมาจาก endpoint แบบอ่านอย่างเดียว
> ตัวเดียวกับที่เว็บแอป claude.ai ใช้วาดมิเตอร์การใช้งานของตัวเอง — ไม่มี prompt
> ไม่เรียกโมเดล ไม่กระทบหน้าต่าง rate-limit จอทำได้แค่ *รายงาน* โควตาเท่านั้น
> ไม่เคยใช้มันเลย ตั้งใจออกแบบมาแบบนี้: การถาม Claude ว่า "ใช้ไปเท่าไหร่แล้ว"
> จะเสียหนึ่งข้อความ และไปเพิ่มตัวเลขที่เรากำลังพยายามเฝ้าดูอยู่พอดี
> ([วิธีทำงาน](#มันได้ตัวเลขมาอย่างไร))
...
token กับ TZ ถึงอยู่ใน `secrets.h` เพราะ token เป็น credential แม้จะเป็นแบบอ่อน
...
(เหตุผลที่ตัวนี้อ่าน endpoint แทนที่จะถาม Claude ตามที่บอกไว้ด้านบน)
...
กระทบแค่สองหน้า Claude มีคำขอเดียววิ่งอยู่ในเวลาใดก็ตาม — TLS handshake พีคราว
45KB สองอันซ้อนกันไม่พอที่ข้างๆ draw buffer
...
- Weather กับคริปโตข้ามการตรวจ certificate โดยตั้งใจ ทั้งคู่ไม่มี credential
