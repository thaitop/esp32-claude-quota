...
| **Setting** | สถานะลิงก์ สุขภาพแต่ละ feed ความเก่าของโควตา heap uptime ความสว่าง Config | — |
...
เกณฑ์สี, ค่าคาลิเบรตทัช — อยู่ใน `firmware/src/config.h` ที่ commit เข้า repo
ที่ token กับ TZ อยู่ใน `secrets.h` ก็เพราะ token เป็น credential ถึงจะเป็นแบบ
อ่อนก็ตาม ค่าตำแหน่งอากาศ เหรียญ และหุ้นใน `config.h`/`secrets.h` เป็น **ค่าตั้งต้น
จากโรงงาน** เมื่อแฟลชแล้ว สามค่านี้แก้บนเครื่องผ่าน [Config Mode](#config-mode)
ได้โดยไม่ต้องแฟลชใหม่
...
หน้า Setting ส่วนใหญ่เป็นหน้าอ่านค่า บวกกับ stepper ปรับความสว่างและปุ่มเดียว คือ
**Config** ที่ขอบล่าง ซึ่งเปิด [Config Mode](#config-mode) ค่าปรับแต่งส่วนใหญ่ยัง
เปลี่ยนได้ด้วยการแฟลชใหม่เท่านั้น Config Mode เป็นข้อยกเว้นสำหรับสามค่าที่คนน่าจะ
เปลี่ยนโดยไม่มี toolchain อยู่ใกล้มือ — ตำแหน่งอากาศ เหรียญที่ติดตาม และหุ้นที่ติดตาม

## Config Mode

แก้ตำแหน่งอากาศ เหรียญ และหุ้นจากเบราว์เซอร์ — ไม่ต้องแฟลชใหม่

**กดค้างปุ่ม Config** บนหน้า Setting feed จะหยุด เครื่องตั้ง HTTP server ธรรมดาขึ้นมา
แล้วจอแสดงที่อยู่และ **PIN** สี่หลัก:

1. บนมือถือหรือแล็ปท็อปที่อยู่ LAN เดียวกัน เปิดที่อยู่ที่แสดง (เช่น
   `http://192.168.1.42`) แล้วใส่ PIN
2. พิมพ์ชื่อเมืองสำหรับอากาศ และเลือกเหรียญกับหุ้นจากเมนู ชื่อเมืองถูกตรวจสดกับ
   Open-Meteo ส่วนเหรียญกับหุ้นเลือกจากแคตาล็อกที่ฝังมา โลโก้จึงตรงเสมอ
3. **Save & reboot** เขียนค่าที่เลือกลงแฟลช (NVS) แล้วรีสตาร์ตเข้าชุดใหม่
   **Cancel** ทิ้งแล้วรีบูต หากต้องออกโดยไม่มีเบราว์เซอร์ — เช่น เผลอแตะเข้ามา —
   **กดค้างที่ไหนก็ได้บนจอ** เพื่อออกโดยไม่บันทึก
