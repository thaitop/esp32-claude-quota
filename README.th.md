...
ค่านั้นคือคุกกี้ `sessionKey` จากเบราว์เซอร์ที่ล็อกอิน claude.ai อยู่
(dev tools → Application → Cookies → `https://claude.ai`) การล็อกอินแบบ OAuth
ของ Claude Code ใช้ **ไม่ได้** — endpoint ตัวนี้ยืนยันตัวตนด้วยคุกกี้ สคริปต์จะ
ปฏิเสธไฟล์คีย์ที่ group หรือ world อ่านได้ ดังนั้น `chmod` ไม่ใช่ทางเลือก
